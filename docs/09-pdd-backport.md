...
## Ref2VA（i2v 路线）同样落地（2026-09-04 补测）

同方法搬到 i2v（Ref2VA-Acc-8Step LoRA，首帧锚定锁脸路线），同 seed 同默认首帧，master 环境热跑：

| 配置 | 耗时 | 说明 |
|---|---|---|
| i2v Turbo 4 步（无 T8，master 重测基线） | 395s | 0.33.1 时为 417s，master 略快 |
| i2v Ref2VA PDD 8 步（无 T8） | 933s | 可复现 + 蒸馏画质 |
| **i2v Ref2VA PDD + T8（阈值 1.0）** | **192s** | **命中 6/8，−51%** |

三个看点：① **192s 是全项目最快纪录**——锁脸这条最贵的路线，现在比 t2v 旧成片档（280s）还快；② 绝对节省 203s/镜，仍是"镜头越贵省得越多"；③ 加载零报错，Ref2VA 版与 FL2VA 版在 master 上行为一致。

## 档位体系（2026-09-04 起，六件套）

| 档位 | 配置 | 速度 | 用途 |
|---|---|---|---|
| ⚡ 极速草稿 | **PDD 8步 + T8** | **210s（t2v）/ 192s（i2v）** | 选镜头/试 prompt，画质最高的草稿 |
